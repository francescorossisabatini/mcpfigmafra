...
import { createColorStyle } from 'tools/create/create-color-style';
import { getLocalVariables } from 'tools/read/get-local-variables';
import { setFillVariable } from 'tools/update/set-fill-variable';
import { setStrokeVariable } from 'tools/update/set-stroke-variable';
  ...
  on<StartTaskHandler>('START_TASK', async function (task: StartTaskHandler) {
    try {
      ...
      if (task.command === 'create-color-style') {
        result = await safeToolProcessor<CreateColorStyleParams>(createColorStyle)(task.args as CreateColorStyleParams);
      }

      if (task.command === 'get-local-variables') {
        result = await safeToolProcessor<GetLocalVariablesParams>(getLocalVariables)(task.args as GetLocalVariablesParams);
      }

      if (task.command === 'set-fill-variable') {
        result = await safeToolProcessor<SetFillVariableParams>(setFillVariable)(task.args as SetFillVariableParams);
      }

      if (task.command === 'set-stroke-variable') {
        result = await safeToolProcessor<SetStrokeVariableParams>(setStrokeVariable)(task.args as SetStrokeVariableParams);
      }
